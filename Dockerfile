...
RUN apt-get update -y && apt-get install -y libstdc++6 openssl libncurses5 locales imagemagick libmagick++-dev libmagic-dev ffmpeg\
...
COPY --from=builder --chown=nobody:root /app/_build/prod/rel/the_little_thinkers_space ./
