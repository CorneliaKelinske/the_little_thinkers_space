defmodule TheLittleThinkersSpaceWeb.RemoveCrewRequestControllerTest do
  ...
  @valid_params %{
    ...
    remove_crew_first_name: "Bumba",
    remove_crew_last_name: "Oyeah",
  ...
  describe "new" do
    test "redirects to login when user is not logged in", %{conn: conn} do
      conn = get(conn, Routes.remove_crew_request_path(conn, :new))
  ...
  test "renders the request form when user is logged in", %{conn: conn, user: user} do
    conn =
      conn
      |> log_in_user(user)
      |> get(Routes.remove_crew_request_path(conn, :new))

    assert html_response(conn, 200) =~ "Remove a crew member"
  end

  describe "create" do
    test "redirects to login if user is not logged in", %{conn: conn} do
      conn =
        post(conn, Routes.remove_crew_request_path(conn, :create),
          remove_crew_request_content: @valid_params
        )
    ...
    test "delivers request and redirects to home when valid params are provided by the logged in user",
      ...
      conn =
        conn
        |> log_in_user(user)
        |> post(Routes.remove_crew_request_path(conn, :create),
          remove_crew_request_content: @valid_params
        )
    ...
    test "renders errors when invalid params are provided by the logged in user", %{
      ...
      conn =
        ...
        |> log_in_user(user)
        |> post(Routes.remove_crew_request_path(conn, :create),
          remove_crew_request_content: @invalid_params
        )
